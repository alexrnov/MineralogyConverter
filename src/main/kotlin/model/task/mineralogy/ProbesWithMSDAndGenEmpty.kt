 ...
 */
class ProbesWithMSDAndGenEmpty
...
constructor(parameters: Map<String, Any>): GeoTaskManyFiles(parameters) {
  ...
  override fun perform(file: File) {
    try {
      ...
      Mineralogy.logger.info("Error write data to output file: ${e.message}")
      throw GeoTaskException("Ошибка записи данных в файл")
    } catch (e: Exception) {
      Mineralogy.logger.info(e.message)
  ...
  override fun printIntro() {
    ...
    if (useReferenceVolume) {
      task.printConsole("Пересчитать количество зерен МСА: Да")
      task.printConsole("Объем пробы: $probeVolume л; ")
    } else {
      task.printConsole("Пересчитать количество кристаллов МСА: Нет")
    }
